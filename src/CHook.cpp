	...
	switch (event->type) {
		case KeyPress:
		{
			...
			switch (XLookupKeysym(&event->xkey, 0))
			{
				...
				{
					printf("cgs dump\n");
					unsigned char *p = (unsigned char*)(0x33e7ae0 + (unsigned long)eth32.cg.module);
					for (; p < (unsigned char*)(0x33e7ae0 + (unsigned long)eth32.cg.module) + 0x1000 ; p++)
					{
						printf("Byte offset: %p\tNumber: %d\tByte: 0x%02X\tAscii: %c\tInt: %d\tUnsigned Int: %u\tFloat: %f\n", p - (unsigned char*)(0x33e7ae0 + (unsigned long)eth32.cg.module), *p, *p, (*p == 0) ? ' ' : *p, *p, *(int*)p, *(float*)p);
					}
					break;
				}
				default:
...
void CHook::ModifyCgameCommands()
{
	...
	// Handle the consolecommand struct change for newer NQ Mods
	if(eth32.cgMod->type == MOD_NQ)
	{
		nqconsoleCommand_t *cc = (nqconsoleCommand_t*)((uint32)eth32.cg.module + eth32.cgMod->commands);
...
void CHook::SaveWeapons(const char *filename)
{
	...
	for (int i=0 ; i<num ; i++)
	{
		...
		if (weap->attribs & WA_USER_DEFINED) {	// only save weapon settings that allowed to be modified
			sprintf(buffer, "%i %i %i %i %s", weap->range, weap->delay, weap->headTraces, weap->bodyTraces, (weap->autofire)?"true":"false");
			sprintf(q,"weap%02i", i);
			Tools.WritePrivateProfileString("No Quarter Weapons", q, buffer, (char *)filename);
		}
	}
	num = sizeof(nqWeapons) / sizeof(weapdef_t);
	for (int i=0 ; i<num ; i++)
	{
		weap = &nqWeapons[i];
		if (weap->attribs & WA_USER_DEFINED) {	// only save weapon settings that allowed to be modified
...
void CHook::LoadWeapons(const char *filename)
{
	...
	for (int i=0 ; i<num ; i++)
	{
		...
		if (weap->attribs & WA_USER_DEFINED) {	// only load weapon settings that allowed to be modified
			sprintf(q, "weap%02i", i);
			Tools.GetPrivateProfileString("No Quarter Weapons", q, "10000 0 4 4 false", buffer, 256, (char *)filename);
			sscanf(buffer, "%i %i %i %i %s", &weap->range, &weap->delay, &weap->headTraces, &weap->bodyTraces, autofire);
			weap->autofire = (strcmp(autofire, "false") != 0);
		}
	}
...
int CDECL eth32_CG_syscall(int command, ...)
{
	...
	switch (command)
	{
		...
		case CG_CVAR_REGISTER:
			...
			// Third Person unlocker for all mods, get the cvars address and then modify elsewhere ;)
			if( !strcmp( (char*)arg[1], "cg_thirdPerson" ) )
			{
				eth32.cgMod->cg_thirdPerson = (unsigned long)arg[0];
			}
...
{
	if ((eth32.cgMod->type != MOD_TCE) && IS_WEAPATTRIB(lastweap, WA_SCOPED) && IS_WEAPATTRIB(newweap, WA_UNSCOPED) && !(Syscall.isKeyActionDown("+reload") || Syscall.isKeyActionDown("weapalt") || Syscall.isKeyActionDown("+attack2")))
			return;
...
void CDECL eth32_CG_ChargeTimesChanged(void)
{
	...
	index = (eth32.cgMod->type == MOD_NQ) ? 0x23 : 0x26;
