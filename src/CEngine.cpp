...
void CEngine::CG_Init_Pre(int serverMessageNum, int serverCommandSequence, int clientNum)
{
	...
	else if (eth32.cgMod->type == MOD_NQ)
		eth32.cg.ws_max = WS_NQ_MAX;
	...
	// Picmip hack
	if( eth32.settings.r_picmip )
	{
		cvar_t *eth32_picmip = *(cvar_t**)eth32.et->r_picmip;
		eth32_picmip->integer = eth32.settings.r_picmipVal;
	}
}
...
void CEngine::CG_Init_Post(int serverMessageNum, int serverCommandSequence, int clientNum)
{
	...
	typedef struct {
        ...
        cvarTable_t *cv = (cvarTable_t*)((unsigned long)eth32.cg.module + 0x1DA030);

	for ( int i = 0; i < 20; i++, cv++ )
	{

		printf( "%s\n", (char*)((unsigned long*)cv) );
	}
}
...
void CEngine::ParseWeaponStats()
{
	...
	int kills;
	// dont count sniper weapons (i guess for now?)
	// All mods share the first 7 weaponstat numbers
	...
	{
		player->stats.bulletsFiredWS += ws[NQWS_STG44].shots + ws[NQWS_BAR].shots + ws[NQWS_MP34].shots;
		player->stats.bulletsHitWS += ws[NQWS_STG44].hits + ws[NQWS_BAR].hits + ws[NQWS_MP34].hits;
		player->stats.headshotsWS += ws[NQWS_STG44].headshots + ws[NQWS_BAR].headshots + ws[NQWS_MP34].headshots;
		kills += ws[NQWS_STG44].kills + ws[NQWS_BAR].kills + ws[NQWS_MP34].kills;
	}
...
void CEngine::PostFrame(void)
{
	...
	// Check to see if we want client cvars modified
	if( eth32.cgMod->cg_thirdPerson )
	{
		vmCvar_t *eth32_cg_thirdPerson = (vmCvar_t*)( eth32.cgMod->cg_thirdPerson );
		if( eth32.settings.thirdPerson && ( eth32_cg_thirdPerson->integer == 0 ) )
			eth32_cg_thirdPerson->integer = 1;
		else if( !eth32.settings.thirdPerson && ( eth32_cg_thirdPerson->integer == 1 ) )
			eth32_cg_thirdPerson->integer = 0;
	}
}
...
int CEngine::CG_DrawActiveFrame(int serverTime, int stereoView, int demoPlayback)
{
	...
	vec4_t color = { 0.5, 0.5, 0.5, 0.5 };

	//char buf[100];
	//sprintf( buf, "Num: %d Weapon: %d", eth32.cg.numWeapons, eth32.cg.snap->ps.weapon );
	//Draw.TextCenter(200, 200, 0.15, buf, color, qfalse, qfalse, GUI_FONT);
